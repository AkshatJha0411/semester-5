...
We built a dense crowd surveillance pipeline using a fine-tuned **YOLO** model and **BoT-SORT** on the **MOT20** dataset. It gave me a much deeper understanding of how vision systems behave outside clean benchmarks.
...
4.  **Learning:** **IL** (Imitation Learning) and **RL** (Reinforcement Learning) controllers for a 3-link robot arm, including training and evaluation in unseen environments.
...
## DP of Mechatronics
We developed a low-cost pressure monitoring aid for bedsore prevention using **FSR** (Force Sensing Resistor) embedded strips. The system continuously measures pressure and triggers alerts when thresholds are exceeded for unsafe durations. The goal was to create an accessible, affordable alternative to existing solutions.
